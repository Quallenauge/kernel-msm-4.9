...
{
	int rc = 0;
	struct bq25892_charger *chip = power_supply_get_drvdata(psy);
...
//	                             chip->parallel_psy_d.name);
//	if (rc)
		chip->parallel_psy_d.name = "parallel";

	chip->parallel_psy_d.type		= POWER_SUPPLY_TYPE_PARALLEL;
	chip->parallel_psy_d.get_property	= bq25892_parallel_get_property;
	chip->parallel_psy_d.set_property	= bq25892_parallel_set_property;
	chip->parallel_psy_d.properties	= bq25892_parallel_properties;
